...
class UptimeKumaServer {
    ...
    async sendMonitorList(socket) {
        let list = await this.getMonitorJSONList(socket);
    ...
    async sendUpdateMonitorIntoList(socket, monitorID) {
        let list = await this.getMonitorJSONList(socket, monitorID);
    ...
    /**
     * Get the monitor list the socket user can view.
     * @param {object} socket Socket with userID and userRole.
     * @param {number} monitorID Optional specific monitor id to look up.
     * @returns {Promise<object>} Map of monitor id -> monitor JSON.
     */
    async getMonitorJSONList(socket, monitorID = null) {
        const { accessibleMonitorIds, canAccessMonitorBean } = require("./monitor-access");
